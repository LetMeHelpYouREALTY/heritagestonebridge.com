        ...
        <div className="max-w-3xl mx-auto">
          {faqs.map((faq, index) => (
            <details
              ...
              open={index === 0}
            >
              <summary className="w-full px-6 py-4 text-left flex justify-between items-center hover:bg-slate-50 transition-colors cursor-pointer list-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-inset [&::-webkit-details-marker]:hidden">
                <span className="font-semibold text-slate-900 pr-4">
                  {faq.question}
                </span>
                <ChevronDown
                  className="h-5 w-5 text-slate-400 flex-shrink-0 transition-transform group-open:rotate-180 group-open:text-blue-600"
                  aria-hidden="true"
                />
              </summary>
